...
class publicview extends file
{
	...
	function statement_result($type=MYSQLI_ASSOC){
		return $this->statement->get_result()->fetch_all($type);
	}

	function get_array($type='assoc'){		
		return $this->statement->get_result()->{'fetch_'.$type}();
    }
	...
	function Get_profile_image($user_id=null)
	{
		if 	((isset($_SESSION['id']) &&!empty($_SESSION['id'])) || $user_id!=null){   
				(($user_id)?$id=$user_id:$id=$_SESSION['id']);
					...
					$row = $this->get_data(array('sex','profile_image','username'),'users','id',$id,'');					
	...
	function insert($table,$column,$values,$attr=array()){			
	...
	function total($table=null,$column=null,$placeholder=null,$value=null,$combine='AND',$join=array(),$attr=array())
	{
		...
		if($join){			
			$this->query .=$this->implode_join($join);
		}	
	...
	function get_data($data=null,$table,$placeholder=null,$conditions=null,$combine='AND',$join=array(),$attr=array()){    
		if (empty($table)) 
		return null;
		$requireddata=(!$data)?'*':$this->implode_array($data,'',',');
		$row_condition=$this->implode_array($placeholder,($conditions!=null)?'?':'',$combine);	
		$this->query ="SELECT $requireddata FROM $table ";
		if($join)			$this->query .=$this->implode_join($join);
		if($row_condition)	$this->query .= " WHERE ".$row_condition;			
		if(!isset($attr['nolimit']))$this->query .= " LIMIT 1";
		if(isset($attr['debug']))	return $this->query;
	...
	function getAllArray($table,$column=null,$value=null,$combine='AND',$limit=null,$orderby=null,$order='DESC',$join=array(),$attr=array()){	//gives all array
		$compare=isset($attr['compare'])?$attr['compare']:'=';
		$row_condition=$this->implode_array($column,($value!=null)?'?':'',$combine,$compare);
		$fields='*';
		if(isset($attr['fields'])) $fields= $this->implode_array($attr['fields'],'',',');
		$this->query =" SELECT $fields FROM $table ";
		if($join)							$this->query .=	  $this->implode_join($join);				
		if(isset($attr['whereparameter']))	$this->query .= " WHERE ".$this->implode_join($attr['whereparameter'],'(');
		elseif(!empty($row_condition))		$this->query .= " WHERE ".$row_condition;
		if(isset($attr['groupby']))			$this->query .= " GROUP BY ".$attr['groupby'];
		if(isset($attr['having']))  		$this->query .= " HAVING ".$attr['having'];
		if(!empty($orderby))  	   			$this->query .= " ORDER BY ".$this->implode_array($orderby,$order,",",""," ","");
		if(!empty($limit))	 	  			$this->query .= " LIMIT ".$limit;
		if(isset($attr['debug']))	return	$this->query;
		$this->execute($value);
		if(isset($attr['array']))	return 	$this->get_array(isset($attr['fetch'])?$attr['fetch']:'assoc');			  
		return $this->statement_result(isset($attr['fetch'])?$attr['fetch']:MYSQLI_ASSOC);
	  }

	function Delete($table,$column=NULL,$value=NULL,$combine='AND',$attr=array()){
		$compare=isset($attr['compare'])?$attr['compare']:'=';
		$row_condition=$this->implode_array($column,($value!=null)?'?':'',$combine,$compare);
		$this->query ="DELETE FROM $table WHERE $row_condition" ;
		if(isset($attr['orderby']))  	 $this->query .= " ORDER BY ".$this->implode_array($attr['orderby'],isset($attr['order'])?$attr['order']:'',",",""," ","");
		if(isset($attr['limit']))	 	 $this->query .= " LIMIT ".$attr['limit'];
		if(isset($attr['debug'])) return $this->query;			
		...
		if ($combine)
			if(is_array($combine)){
				...
				foreach($finalarray as $key=>$returnarray){
					if(array_key_exists($count, $combine))
